...
class MarkdownRenderer {
    constructor() {
      this.rules = [
        // Headers
        { pattern: /^### (.*$)/gm, replacement: '<h3 class="text-2xl font-bold mb-4 mt-6 text-textLight dark:text-textDark">$1</h3>' },
        { pattern: /^## (.*$)/gm, replacement: '<h2 class="text-3xl font-bold mb-4 mt-8 text-textLight dark:text-textDark">$1</h2>' },
        { pattern: /^# (.*$)/gm, replacement: '<h1 class="text-4xl font-bold mb-6 mt-8 text-textLight dark:text-textDark">$1</h1>' },
        
        // Bold and Italic
        { pattern: /\*\*\*(.*?)\*\*\*/g, replacement: '<strong class="font-bold"><em>$1</em></strong>' },
        { pattern: /\*\*(.*?)\*\*/g, replacement: '<strong class="font-bold">$1</strong>' },
        { pattern: /\*(.*?)\*/g, replacement: '<em>$1</em>' },
        { pattern: /\_\_\_(.*?)\_\_\_/g, replacement: '<strong class="font-bold"><em>$1</em></strong>' },
        { pattern: /\_\_(.*?)\_\_/g, replacement: '<strong class="font-bold">$1</strong>' },
        { pattern: /\_(.*?)\_/g, replacement: '<em>$1</em>' },
        
        // Code blocks
        { pattern: /```([\s\S]*?)```/g, replacement: '<pre class="bg-black dark:bg-white text-white dark:text-black p-4 rounded-sm overflow-x-auto my-4 border-2 border-borderLight dark:border-borderDark"><code>$1</code></pre>' },
        { pattern: /`(.*?)`/g, replacement: '<code class="bg-black dark:bg-white text-white dark:text-black px-2 py-1 rounded-sm font-mono text-sm">$1</code>' },
        
        // Links
        { pattern: /\[(.*?)\]\((.*?)\)/g, replacement: '<a href="$2" class="underline hover:no-underline font-bold">$1</a>' },
        
        // Images
        { pattern: /!\[(.*?)\]\((.*?)\)/g, replacement: '<img src="$2" alt="$1" class="my-4 border-2 border-borderLight dark:border-borderDark rounded-sm" />' },
        
        // Blockquotes
        { pattern: /^> (.*$)/gm, replacement: '<blockquote class="border-l-4 border-borderLight dark:border-borderDark pl-4 my-4 italic text-textMuted dark:text-textMutedDark">$1</blockquote>' },
        
        // Horizontal rule
        { pattern: /^---$/gm, replacement: '<hr class="border-t-2 border-borderLight dark:border-borderDark my-8" />' },
        
        // Unordered lists
        { pattern: /^\* (.*$)/gm, replacement: '<li class="ml-6 mb-2">$1</li>' },
        { pattern: /^- (.*$)/gm, replacement: '<li class="ml-6 mb-2">$1</li>' },
        
        // Ordered lists
        { pattern: /^\d+\. (.*$)/gm, replacement: '<li class="ml-6 mb-2">$1</li>' },
        
        // Paragraphs (must be last)
        { pattern: /^(?!<[^>]+>)(.+)$/gm, replacement: '<p class="mb-4 leading-relaxed text-textMuted dark:text-textMutedDark">$1</p>' }
      ];
    }
  ...
  class ArticleNavigator {
     ...
     */
    async init() {
      // Load articles from the card constructor if available
    ...
    /**
     * Render navigation to previous/next articles
     */
